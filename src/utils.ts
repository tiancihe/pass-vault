...
export function range(start: number, end: number) {
    if (end <= start) {
        throw new Error(
            `range: end: ${end} is not greater than start: ${start}`
        )
    }
...
const buildPickChar = (type: GenerateType) =>
    type === GenerateType["NumbersAndCharacters"]
        ? () =>
              String.fromCharCode(
                  NUMBERS_RANGE.concat(LOWER_CASES_RANGE).concat(
                      UPPER_CASES_RANGE
                  )[
                      Math.floor(
                          Math.random() *
                              (NUMBERS_RANGE.length +
                                  LOWER_CASES_RANGE.length +
                                  UPPER_CASES_RANGE.length)
                      )
                  ]
              )
        : () =>
              String.fromCharCode(
                  NUMBERS_RANGE[
                      Math.floor(Math.random() * NUMBERS_RANGE.length)
                  ]
              )
